...
const INNATE_THRESHOLDS = {
    'HP%': 7, 'DEF%': 7, 'ATK%': 7, 'CRate': 5, 'CDmg': 6, 'ACC': 6, 'RES': 7,
    'HP flat': 149, 'DEF flat': 10, 'ATK flat': 11
};
...
function getPowerLevel(rune) {
    ...
    let count = 0;
    for (let s of stats) if (s.val >= thresholds[s.name]) count++;
    return count;
}
...
function getHighRoll(rune) {
    ...
    for (let s of stats) {
        if (getStatValue(rune, s) >= getStatThreshold(rune, s)) return 'Match';
    }
    return 'No';
}
...
function renderTable() {
    ...
    filtered = filtered.filter(r => currentDecisionFilters.includes(getFinal(r, getHighRoll(r), getDuoRoll(r), getClassicDPS(r), getSlowDPS(r), getBomber(r), getTankSupport(r), getBruiser(r), getReapp(r), getUpgrade(r))));
...
function initSortListeners() {
    ...
    table.addEventListener('click', (e) => {
        ...
        if (idx === -1) return;
        if (currentSort.column === idx) currentSort.asc = !currentSort.asc;
        else { currentSort.column = idx; currentSort.asc = true; }
        renderTable();
    });
}
// ==================== FORMULAIRES MODAL ====================
function buildHighRollFormForLevel(level) {
    ...
    for (let rarity of rarities) {
        ...
        rarityDiv.className = 'settings-rarity';
        rarityDiv.innerHTML = `<h3>${rarity.toUpperCase()}</h3>`;
...
function buildDuoRollFormForLevel(level) {
    const container = document.getElementById('settingsForm');
    if (!container) return;
    container.innerHTML = '';
    const types = ['legend', 'hero'];
    const keys = [
        'SPD_min', 'SPD_partner', 'CRate_for_CDmg', 'CDmg_for_CRate',
        'CRate_for_ATK', 'ATK_for_CRate', 'HP_for_DEF', 'DEF_for_HP',
        'DEF_for_RES', 'RES_for_DEF', 'HP_for_RES', 'RES_for_HP'
    ];
    for (let type of types) {
        const typeDiv = document.createElement('div');
        typeDiv.className = 'duo-type';
        typeDiv.innerHTML = `<h3>${type.toUpperCase()}</h3>`;
        const grid = document.createElement('div');
        grid.className = 'duo-grid';
        for (let key of keys) {
            const val = SETTINGS.duo[type][level][key];
            const label = document.createElement('label');
            label.textContent = key.replace(/_/g, ' ');
            const input = document.createElement('input');
            input.type = 'number';
            input.value = val;
            input.dataset.duoType = type;
            input.dataset.duoLevel = level;
            input.dataset.duoKey = key;
...
function buildSettingsForm() {
    ...
    const activeLevelBtn = document.querySelector('.modal-level-btn.active');
    const currentModalLevel = activeLevelBtn ? activeLevelBtn.dataset.level : currentGameLevel;
    if (activeTab === 'tabDuoRollBtn') {
        buildDuoRollFormForLevel(currentModalLevel);
    } else {
        buildHighRollFormForLevel(currentModalLevel);
    }
}
...
function openModal() {
    ...
    initModalLevelButtons();
}
function closeModal() { document.getElementById('settingsModal').style.display = 'none'; }
function initModalLevelButtons() {
    const buttons = document.querySelectorAll('.modal-level-btn');
    if (!buttons.length) return;
    buttons.forEach(btn => {
        if (btn.dataset.level === currentGameLevel) btn.classList.add('active');
        ...
        btn.addEventListener('click', (e) => {
            ...
            const activeTab = document.querySelector('.tab-btn.active')?.id;
            if (activeTab === 'tabDuoRollBtn') buildDuoRollFormForLevel(level);
            else buildHighRollFormForLevel(level);
        });
    });
}
function initSettingsTabs() {
    ...
    highRollBtn.addEventListener('click', () => {
        ...
        const activeLevelBtn = document.querySelector('.modal-level-btn.active');
        const level = activeLevelBtn ? activeLevelBtn.dataset.level : currentGameLevel;
        buildHighRollFormForLevel(level);
    });
    duoRollBtn.addEventListener('click', () => {
        ...
        const activeLevelBtn = document.querySelector('.modal-level-btn.active');
        const level = activeLevelBtn ? activeLevelBtn.dataset.level : currentGameLevel;
        buildDuoRollFormForLevel(level);
    });
}
